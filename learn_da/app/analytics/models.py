...
class UserProfile(BaseModel):
    ...
    visitor_id = Column(
        String(64), nullable=False, unique=True, index=True, comment="访客 ID"
    )
    ...
    last_active_date = Column(
        String(10), nullable=True, comment="最后活跃日期 YYYY-MM-DD"
    )
...
class DailyStats(BaseModel):
    ...
    stat_date = Column(
        String(10), nullable=False, unique=True, index=True, comment="日期 YYYY-MM-DD"
    )
...
class CodeSnapshot(BaseModel):
    ...
    __table_args__ = (Index("idx_cs_visitor_lesson", "visitor_id", "lesson_slug"),)
